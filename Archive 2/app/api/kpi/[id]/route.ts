...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      // ใช้ query แบบง่ายเพื่อหลีกเลี่ยง error
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      // อัปเดตข้อมูล KPI
      const updateQuery = `UPDATE kpis SET
        type_id = ?,
      ...
      // ใช้ query แบบง่ายที่ไม่มีการจัดการ kpi_type เพื่อหลีกเลี่ยง error
      console.log("PUT /api/kpi/[id] - Using simple update query")
      console.log("PUT /api/kpi/[id] - Update query:", updateQuery)
      console.log("PUT /api/kpi/[id] - Params:", updateParams)
      
      try {
        await conn.execute(updateQuery, updateParams)
        console.log("PUT /api/kpi/[id] - Update successful")
      } catch (updateError: any) {
        console.error("PUT /api/kpi/[id] - Update failed:", updateError)
        throw new Error(`การอัปเดตข้อมูลล้มเหลว: ${updateError.message}`)
      }
