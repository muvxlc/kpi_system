...
export async function POST(request: NextRequest) {
  try {
    ...
    // ตรวจสอบ kpiType (ถ้ามี column kpi_type)
    const kpiType = data.kpiType || 'hospital'

    const conn = await createConnection()

    try {
      // เพิ่มข้อมูล KPI ใหม่ (รองรับกรณีที่ไม่มี column kpi_type)
      let insertQuery: string
      let insertParams: any[]
      
      try {
        // ลองใช้ query ที่มี kpi_type
        insertQuery = `INSERT INTO kpis (
          kpi_type, \`group\`, plan, project, title, titleEn, detail, objective, 
        ...
        // ถ้าไม่มี column kpi_type ให้ใช้ query ที่ไม่มี kpi_type
        insertQuery = `INSERT INTO kpis (
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("GET /api/kpi - Params:", { page, limit, group, dept, kpiType })
    ...
    try {
      ...
      // เพิ่มการกรอง kpiType โดยใช้ type_id
      if (kpiType && kpiType !== "all") {
        console.log("Filtering by kpiType:", kpiType)
        
        if (kpiType === 'hdc') {
          // HDC: type_id = 1
          whereClause += ` AND k.type_id = 1`
          countWhereClause += ` AND type_id = 1`
          console.log("Applied HDC filter (type_id = 1)")
        } else if (kpiType === 'hospital') {
          // Hospital: type_id = 2
          whereClause += ` AND k.type_id = 2`
          countWhereClause += ` AND type_id = 2`
          console.log("Applied Hospital filter (type_id = 2)")
        }
      }
      ...
      console.log("GET /api/kpi - Count query:", countQuery)
      console.log("GET /api/kpi - Where clause:", whereClause)
      ...
      // ดึงรายการประเภท KPI (ใช้ค่า default)
      const kpiTypeRows = [
        { kpi_type: 'hospital' },
        { kpi_type: 'hdc' }
      ]
