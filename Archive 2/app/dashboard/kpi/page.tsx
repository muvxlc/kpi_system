...
async function getKpiList({
  ...
  try {
    ...
    if (kpiType) url.searchParams.append("kpiType", kpiType)
...
export default async function KpiListPage({
  ...
  const kpiTypeParam = params.kpiType
  ...
  const kpiType = typeof kpiTypeParam === "string" ? kpiTypeParam : undefined

  console.log("KpiListPage: Params:", { page, limit, group, dept, kpiType })

  const { kpis, total, groups, departments, kpiTypes } = await getKpiList({ page, limit, group, dept, kpiType })
  
  // ตรวจสอบและกำหนดค่า default สำหรับ kpiTypes
  const safeKpiTypes = kpiTypes || [
    { kpi_type: 'hospital' },
    { kpi_type: 'hdc' }
  ]
  ...
  return (
    <div className="space-y6">
      ...
      <Card>
        ...
        <CardContent>
          <KpiFilter groups={groups} departments={departments} kpiTypes={safeKpiTypes} />
        </CardContent>
      </Card>
