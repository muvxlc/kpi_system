...
export function KpiDetail({ kpi }: KpiDetailProps) {
  // ฟังก์ชันสำหรับกำหนดประเภท KPI
  const getKpiType = (kpi: any) => {
    // ใช้ type_id จากฐานข้อมูล
    if (kpi.type_id === 1) {
      return 'hdc'
    } else if (kpi.type_id === 2) {
      return 'hospital'
    }
    
    // ถ้าไม่มี type_id ให้ใช้การวิเคราะห์จากชื่อหรือกลุ่ม (fallback)
    const title = kpi.title?.toLowerCase() || ''
    const group = kpi.group?.toLowerCase() || ''
    
    if (title.includes('hdc') || title.includes('กระทรวง') || title.includes('สาธารณสุข') ||
        group.includes('hdc') || group.includes('กระทรวง') || group.includes('สาธารณสุข')) {
      return 'hdc'
    }
    
    // ค่า default เป็น hospital
    return 'hospital'
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <h3 className="text-sm font-medium text-muted-foreground">ประเภท KPI</h3>
          <div className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${
            getKpiType(kpi) === 'hdc' 
              ? 'bg-blue-100 text-blue-800' 
              : 'bg-green-100 text-green-800'
          }`}>
            {getKpiType(kpi) === 'hdc' ? 'HDC (กระทรวงสาธารณสุข)' : 'Hospital (โรงพยาบาล)'}
          </div>
        </div>
