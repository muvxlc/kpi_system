  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        // แปลง kpiType เป็น type_id
        const typeId = formData.kpiType === 'hdc' ? 1 : 2
        
        const requestBody = {
          ...formData,
          type_id: typeId, // ส่ง type_id แทน kpiType
        ...
        console.log("kpiType being sent:", formData.kpiType)
        console.log("type_id being sent:", typeId)
        ...
        if (!response.ok) {
          let errorMessage = "Failed to update KPI"
          try {
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-2">
          <Label htmlFor="kpiType">
            ประเภท KPI <span className="text-red-500">*</span>
          </Label>
          <Select
            value={formData.kpiType}
            onValueChange={(value) => handleSelectChange("kpiType", value)}
            required
          >
            <SelectTrigger id="kpiType">
              <SelectValue placeholder="เลือกประเภท KPI" />
            </SelectTrigger>
            <SelectContent>
